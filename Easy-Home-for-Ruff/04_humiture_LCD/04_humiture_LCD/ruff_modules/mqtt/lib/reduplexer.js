...
function Reduplexer(writable, readable, options) {
    if (!(this instanceof Reduplexer)) {
        return new Reduplexer(writable, readable, options);
    }
...
Reduplexer.prototype.hookWritable = function hookWritable(writable) {
    var that = this;
...
Reduplexer.prototype.hookReadable = function hookReadable(readable) {
    var that = this;
    var dummyWritable = new Writable(this._options);
...
Reduplexer.prototype.hook = function hook(writable, readable) {
    if (writable) {
...
Reduplexer.prototype._read = function read() {
...
Reduplexer.prototype._callWrite = function nop() {
    throw new Error('hook Writable to use');
};

Reduplexer.prototype._write = function write(chunk, enc, cb) {
    if (this._writable) {
